...
qkv = rearrange(Wqkv(x), 'b s (t h d) -> t b s h d', t=3, h=nheads).detach().requires_grad_()
q, k, v = qkv[0], qkv[1], qkv[2]

fn_with_dropout = lambda: flash_attn_func(
    q, k, v, attention_dropout=0.1
)
fn_without_dropout = lambda: flash_attn_func(q, k, v, attention_dropout=0.0)
benchmark_all(fn_with_dropout, qkv_unpad, repeats=repeats, desc='With Dropout')
benchmark_all(fn_without_dropout, qkv_unpad, repeats=repeats, desc='Without Dropout')
